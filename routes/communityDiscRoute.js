...
const communityDiscRouter = express.Router();

//Get all discussions
communityDiscRouter.get("/", async (req, res) => {
  try {
...
communityDiscRouter.get("/book/:id", async (req, res) => {
  const { id } = req.params;
  try {
...
communityDiscRouter.get("/:id", async (req, res) => {
  const { id } = req.params;
  try {
...
//Create a discussion
communityDiscRouter.post("/create", authMiddleware, async (req, res) => {
  try {
...
communityDiscRouter.patch("/update/:id", authMiddleware, async (req, res) => {
  const { id } = req.params;
...
communityDiscRouter.delete("/delete/:id", authMiddleware, async (req, res) => {
  const { id } = req.params;
  const { userID } = req.body;
  try {
    const checkDiscussion = await CommunityDiscModel.findOne({ _id: id });
